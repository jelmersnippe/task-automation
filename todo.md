...
- [ ] Fix grammatical error message in `interpreter/scope.rs` `update_variable` — `"Identifier '{}' has not declared"` should read `"Identifier '{}' has not been declared"`.
...
- [ ] Return an error instead of `DataType::Undefined()` from `scope.get_variable` when a variable doesn't exist — silent undefined values cause confusing panics far from the actual mistake.
...
- [ ] Implement `while` loop — no token, AST node, or interpreter support exists yet.
...
- [ ] Design and implement a `register_task(name, fn)` builtin that stores named tasks.
- [ ] Implement a `run(name)` builtin that executes a registered task by name.
